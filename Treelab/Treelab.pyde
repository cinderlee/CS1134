...
class Tree:
    ...
    def flip (self,p):
        #if self.is_leaf (p):
         #   return
        nodester = p._node._left
        p._node._left = p._node._right
        p._node._right = nodester
       # return self.flip (self.left (p)), self.flip (self.right (p))
    ...
    def flip_subtree (self,p = None):
        ...
        p._node._left = p._node._right
        p._node._right = nodester
        return self.flip_subtree (self.left (p)), self.flip_subtree (self.right (p))
    
    def bring_left_up (self, p):
        if p == self.root ():
            leftnode = self.left(p)._node
            keeprightofleftnode = self.left(p)._node._right
            leftnode._right = p._node
            leftnode._parent = None
            p._node.parent = leftnode
            if keeprightofleftnode != None:
                p._node._left = keeprightofleftnode
            else:
                p._node._left = None
            p._node._root = None
            self._root = leftnode
...
def levelTree (n, start = 0):
    T = Tree ()
    if start < n:
        T.attach (T.add_root (n), levelTree (n-1, start), levelTree (n-1, start))
    return T
...
def fractionTree (n, start = 0, top = 1, bot = 2):
    T = Tree ()
    if start < n:
        T.attach (T.add_root (str (top) + "/" + str (bot)), 
                  fractionTree (n-1, start, top * 2 - 1, bot * 2),
                  fractionTree (n-1, start, top * 2 + 1, bot * 2))
    return T
def key2():
    global T
