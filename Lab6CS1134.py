# -*- coding: utf-8 -*-
"""
Created on Fri Mar  3 11:00:31 2017
...
class PList:
    ...
    def split_after (self, p):
        newlist = PList()
        endnode = p._node
        nodester = endnode._next
        newlist._head._next = nodester
        nodester._prev = newlist._head
        newend = self._tail._prev
        endnode._next = self._tail
        newend._next = newlist._tail
        self._tail._prev = endnode
        newlist._tail._prev = newend
        return newlist
        
    def split_before (self, p):
        newlist = PList()
        endnode = p._node._prev
        nodester = p._node
        newlist._head._next = nodester
        nodester._prev = newlist._head
        newend = self._tail._prev
